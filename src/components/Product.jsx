...
const Product = ({handleClick , edit , product : { name , price , discount , image , rating , category}}) => {
  return (
    <div onClick={handleClick} className=" max-sm:w-full w-1/4 h-[450px]  m-2 border-[1px] rounded-t-lg flex flex-col items-center transition-all duration-200 cursor-pointer hover:bg-gray-200 overflow-hidden">
      ...
      <div  className=" w-full flex flex-col items-center mt-4 ">
        ...
        <span className="font-bold text-2xl truncate w-90% text-center">{name}</span>
        ...
        <span className={`${!edit ? 'hidden': "ml-auto px-4 hover:text-red-500"}`}><i class="fa-regular fa-pen-to-square"></i></span>
      </div>
    </div>
  );
};
